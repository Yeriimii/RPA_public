...
def send_mail_to_webhard_for_delete():
    cum_sum = 0
    ...
    for idx, client in enumerate(client_list, 1):
        ...
        df_uploader:pd.Series = pd.read_sql(query_select_uploader, conn)

        for webhard in df_uploader['']:
            df_msg_for_delete = df[df[''] == webhard].iloc[:, 1:]
            if len(df_msg_for_delete) == 0:
                continue
            to_webhard_email = df_uploader[df_uploader[''] == webhard][''].iat[0]

            root_msg = MIMEMultipart('related')
            root_msg['Subject'] = ''
            root_msg['From'] = from_mail_addr
            root_msg['To'] = to_webhard_email
            root_msg['Cc'] = cc_addr
            root_msg.preamble = ''
            ...
            msg_contents = base_html_1 + base_html_client + df_msg_for_delete.to_html(index=False, justify='center', col_space=100) + base_html_2
            ...
            session.sendmail(from_mail_addr, [to_webhard_email, cc_addr], root_msg.as_string())
            update_query = """
                UPDATE
                SET
                WHERE
                """
            fetch_count = curs.executemany(update_query, df[df['']==webhard][''].values.tolist())
...
def send_mail_to_webhard_for_affiliation():
    cum_sum = 0
    session = smtplib.SMTP(host=mail_server, port=mail_port)  # Gmail Port: 587
    session.login(user=login_id, password=login_pw)
    ...
    for idx, client in enumerate(client_list, 1):
        ...
        for webhard in df_uploader['']:
            df_msg_for_affiliation = df[df[''] == webhard].iloc[:, 1:]
            if len(df_msg_for_affiliation) == 0:
                continue
            to_webhard_email = df_uploader[df_uploader[''] == webhard]['email'].iat[0]
            ...
            msg_contents = base_html_1 + base_html_client + df_msg_for_affiliation.to_html(index=False, justify='center', col_space=100) + base_html_2
            text_msg = MIMEText(msg_contents, 'html', _charset="utf-8")
            alternative_message.attach(text_msg)
